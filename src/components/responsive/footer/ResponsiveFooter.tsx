...
const ResponsiveFooter = () => {
  ...
  return (
    <>
      ...
      {isBottomNavOpen && <div onClick={() => setIsBottomNavOpen(false)} className="fixed inset-0 z-10" />}
      ...
      <footer className="fixed bottom-0 left-0 w-full z-30">
        ...
        {!isBottomNavOpen && (
          <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 mb-2 z-30 lg:hidden">
            <div onClick={() => setIsBottomNavOpen(true)} className="w-16 h-2 bg-[#FF5722] rounded-full cursor-pointer" />
          </div>
        )}
